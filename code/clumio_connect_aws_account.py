...
def lambda_handler(events, context):
    ...
    if not all((account_id, region_list, aws_service_list)):
        raise clumio_sdk_v8c.InvalidInputException(
            "Missing account detail. Make sure to provide account_id, region_list, "
            f"and aws_service_list for account {account}."
        )

    region = region_list[0]
    aws_account_mng = clumio_sdk_v8c.AWSOrgAccount(
        api_url, account_id, region, organization_role_arn
    )
    ...
    return f"Stack {stack_name} deployment completed for {account_id=}, {region=}, {aws_service_list=}"
